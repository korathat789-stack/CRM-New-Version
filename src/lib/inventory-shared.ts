// Warehouse inventory: pure helpers + category/status metadata that are safe
// to import from client components (no server-only deps). Data loaders that
// touch Supabase live in `./inventory`, which re-exports these for server
// call sites so existing imports keep working.
